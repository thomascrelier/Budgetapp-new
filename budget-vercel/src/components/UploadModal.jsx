    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          <motion.div
            className="relative glass-card rounded-xl shadow-2xl w-full max-w-md mx-4"
          ...
          >
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <motion.div
                  className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-accent/40 transition-colors"
                  animate={!selectedFile ? { boxShadow: ['0 0 0 0 rgba(212,168,83,0)', '0 0 0 8px rgba(212,168,83,0.1)', '0 0 0 0 rgba(212,168,83,0)'] } : {}}
                  transition={!selectedFile ? { duration: 2, repeat: Infinity, ease: 'easeInOut' } : {}}
                >
            ...
            <div className="px-6 py-4 border-t border-border/50 flex justify-end gap-3">
              <motion.button whileTap={{ scale: 0.97 }} onClick={onClose} className="px-4 py-2 text-text-secondary hover:text-text-primary transition-colors">
                {result ? 'Close' : 'Cancel'}
              </motion.button>
              {!result && (
                <motion.button whileTap={{ scale: 0.97 }} onClick={handleUpload} disabled={!selectedFile || uploading} className="px-4 py-2 bg-accent text-background rounded-lg font-semibold hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm">
